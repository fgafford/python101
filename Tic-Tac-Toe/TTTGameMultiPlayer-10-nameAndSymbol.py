...
# player 1
p1 = {'name':'player 1', 'symbol':'X'}
p2 = {'name':'player 2', 'symbol':'O'}
...
# Asks player for their name and sets it for them
def askAndSetPlayerName(player):
	print(player['name'] + ' please enter your name:')
	player['name'] = input()
	while True:
		print(YEL+player['name']+END + ' please enter the symbol you want to use:')
		txt = input()
		if len(txt) != 1:
			print('Symbol must be only 1 character')
			continue
		else:
			player['symbol'] = txt
			break
...
askAndSetPlayerName(p1)
askAndSetPlayerName(p2)

# Start turns by printing empty board
printBoard(board)

# The actual game loop!
for c in range(9):
	if c%2 == 0:
		currentPName = p1['name']
		currentPSymbol = p1['symbol']
	else:
		currentPName = p2['name']
		currentPSymbol = p2['symbol']
